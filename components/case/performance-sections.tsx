...
// ✅ SINGLE IMPORT!
import { CaseData, Locale, getLocalizedField, getLocalizedArray } from '@/types';
...
export function PerformanceSections({ caseData, locale }: PerformanceSectionsProps) {
  ...
  const solution = getLocalizedField(caseData, 'solution', localeTyped);
  const learnings = getLocalizedArray(caseData, 'key_learnings', localeTyped);
      ...
      <section className="px-6 py-16 lg:px-8">
        <AnimatedSection className="mx-auto max-w-4xl">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {caseData.metrics.map((metric, index: number) => {
              // ✅ Type-safe extraction inline
              const label = locale === 'pt' ? metric.label_pt :
                locale === 'en' ? metric.label_en :
                locale === 'es' ? metric.label_es :
              ...
              const description = locale === 'pt' ? metric.description_pt :
                ...
                locale === 'es' ? metric.description_es :
      ...
      <section className="px-6 py-16 lg:px-8">
        <AnimatedSection className="mx-auto max-w-4xl">
          ...
          <ul className="space-y-4">
            {learnings.map((learning, i: number) => (
